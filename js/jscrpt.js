 var app= angular.module("myapp",[]);
   //var app=angular.module("myapp");
           ...
           app.controller("MyController", function($scope,$http) {
				...
				$http.post("registration1.php",{'fname': $scope.myForm.firstName,'lname': $scope.myForm.lastName,'uname': $scope.myForm.userName,'email': $scope.myForm.email,'password': $scope.myForm.password,'confirmpassword': $scope.myForm.confirmpassword})
       ...
       app.controller("MyController1", function($scope,$http,$window) {
			 ...
			 $http.post("login.php",{'uname':$scope.myForm.userName,'password': $scope.myForm.password})
			  .success(function(data,status,headers,config){
				  ...
				  $window.location.href='admin?uname=' + $scope.myForm.userName;
				 //alert(data.message);
				  }
				 else if(data.message=="You are logged in as user"){
					 $window.location.href='user?uname='+ $scope.myForm.userName ;
					 // alert(data.message);
				 }
